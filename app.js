...
app.post('/getAllMsgs', function(req, res) {	
	pool.getConnection(function(err, connection) {	
		var string = 'SELECT s.*, COUNT(m.id_mensagem) FROM (SELECT  m.*, curtido FROM mensagem m LEFT JOIN mensagem_x_usuario  ON m.id = mensagem_x_usuario.id_mensagem AND mensagem_x_usuario.id_usuario = '+req.body.id_usuario+') s LEFT JOIN mensagem_x_usuario m  ON s.id = m.id_mensagem GROUP BY s.id ORDER BY s.data_criacao desc LIMIT 0,15';		
		console.log(string);
		connection.query(string, function(err, data) {
			if (err){
				var error = {};
