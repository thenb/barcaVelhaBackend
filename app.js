...
app.post('/getAllEnquetes', function(req, res) {	
	pool.getConnection(function(err, connection) {	
		...
		var string = 'SELECT s.*, (select resposta from enquete_x_usuario where id_usuario = '+req.body.id_usuario+' and id_enquete = s.id) as resposta FROM enquete as s ORDER BY s.respondido asc, s.data_criacao desc  LIMIT 0,15';
